...
mysql -u root -p$root_pass -h localhost -e "CREATE DATABASE $database;"
mysql -u root -p$root_pass -h localhost -e "CREATE USER '$db_user'@'localhost' IDENTIFIED BY '$db_user_pass';"
mysql -u root -p$root_pass -h localhost -e "GRANT ALL PRIVILEGES ON $database.* TO '$db_user'@'localhost';"
